...
function replaceText(word, replacement, text) {
  ...
  const replacText = text.replaceAll(regexp, replacement);
  return replacText;
}
...
function extractTextInParentheses(str) {
  ...
  const regexp = /\((.*?)\)/g;
  const matched = str.matchAll(regexp);
  const arrMatched = Array.from(matched, (match) => match[1]);
  return arrMatched;
}
...
function countEmails(str) {
  ...
  const email = matched ? matched.length : 0;
  return email;
}
...
function checkRegexFlags(regex) {
  ...
  const haveFlags = regex.flags;
  if (haveFlags.includes("g") && haveFlags.includes("m")) {
    return true;
  } else {
    return false;
  }
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const regexp = new RegExp(word, "g");
  const replaceAll = str.replaceAll(word, newWord);
  return replaceAll;
}
...
function checkRegexMethods(regex) {
  ...
  if (regex.dotAll && regex.multiline && regex.sticky) {
    arrMethod.push("dotAll", "multiline", "sticky");
  }
  return arrMethod;
}
...
function findWord(str, word) {
  ...
  const searchWord = str.search(regex);
  return searchWord;
}
